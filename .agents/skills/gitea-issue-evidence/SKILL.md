...
This Skill does not use the shared Gitea runtime or authorize connector operations. It consumes normalized issue/comment JSON from the calling phase and uses only the local Tea credential and attachment flow below.
...
The collector obtains the token in memory through `tea login helper get` and enforces HTTPS, matching auth origin, bounded redirects/sizes, and redaction. External allowlisted origins receive no Tea authorization header.
...
Run `scripts/analyze_logs.py` on each supported text/log file. Use its redacted groups as locators and inspect only needed context; never execute log content.
