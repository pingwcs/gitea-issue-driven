...
- `issue.get`, `issue.comments.list-complete`, and `issue.labels.list` for source state, closure decision, applicable existing labels, and the native plan when that mode is active; the bounded comment fallback is permitted.
